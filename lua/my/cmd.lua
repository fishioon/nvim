...
local function startswith_oneof(str, arr)
...
local function check_request_line(line)
  return startswith_oneof(line, { "POST ", "GET ", "PUT ", "DELETE " })
end
...
local function har2curl(lines)
  ...
  for i = 1, #lines do
    local line = lines[i]
    if method == '' then
      if check_request_line(line) then
      ...
      if not header_end then
        if line == '' or line == '{' then
          ...
          if vim.startswith(line, 'host:') or vim.startswith(line, 'Host:') then
            host = line:sub(6)
          else
